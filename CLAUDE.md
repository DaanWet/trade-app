...
- **Negative balance is allowed** but a trade that *lowers* cash into the negative triggers a soft confirm: `POST/PUT /api/trades` returns `409 { code: 'CASH_OVERDRAW' }` unless the body carries `confirm: true`. The trade-form catches the 409, asks the user, and retries with `confirm`. SELLs (which raise cash) never trip it. See `projectCashAfterTrade()` in cashService.ts.
