...
```
backend/src/
  index.ts          — Express entry, mounts 5 routers
...
frontend/src/app/
  ...
  utils/format.ts               — formatMoney, formatNumber, formatPercent, formatShares, formatDate, pnlClass (nl-BE locale)
  shared/
...
## Native module tip
`better-sqlite3` is a native module. After switching Node versions or copying `node_modules` between machines you may see `NODE_MODULE_VERSION` errors at startup. Fix: `npm rebuild better-sqlite3` (or nuke `node_modules` and `npm install`).
